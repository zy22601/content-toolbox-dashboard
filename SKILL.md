...
description: 搭/维护本机+公开的内容工作台 dashboard（16 工具 + R2 图床 + 知识库），零成本 Cloudflare 托管。触发：搭工具箱、做图床、内容工具站、工具化、轻作。
version: 1.1.0
author: zy22601
license: MIT
metadata:
  hermes:
    tags: [content-toolbox, dashboard, r2, image-host, cloudflare-pages, markdown, wechat, qingzuo]
    related_skills: [web-design-engineer, skills-install, proxy-git-and-updates, cloudflare-pages-deploy]
---

# 轻作 · 内容工作站 · Content Toolbox Dashboard

> 品牌名 **「轻作」(QingZuo)**，口号「轻松做内容」。侧栏 logo、title、README、GitHub 均已更名为轻作。在线体验（公开演示版）：**https://qingzuo.toolscomb.com**

把写内容高频用到的工具打包成一个**本机/公开都可用**的单页 dashboard：**16 个工具**（R2 图床、Markdown→PNG、图文卡片、去 Emoji、公众号排版、封面生成、字数统计、二维码生成、九宫格切图、图片压缩、图片加水印、热榜选题、一键分发、AI 周报、违禁词检测、**知识库**）+ 设置。零后端、零成本、全离线可跑（R2 图床、热榜选题、AI 选题需联网）。

**路径约定**：本文用 `$CTB` 指代项目根目录（作者默认 `~/workspace/content-toolbox`，本仓库即源码，请按你的环境替换）。标注「本机/作者专属」的路径是个人部署细节，仅在你复刻作者本机环境时需要，公共化安装可忽略。

## 何时使用（When to Use）

- 用户说「把我的常用内容工具做成 dashboard / 工具箱 / 工具站」并希望**单文件 + 零成本 + 可公开演示**
- 想做图床（R2 / Cloudflare）并带管理界面、拿直链/Markdown/HTML
- 把高频操作（Markdown 转图、公众号排版、去 emoji、做封面/卡片/二维码）合成一个页面
- 部署纯前端工具到 Cloudflare Pages / Netlify，并让**公开演示页**在无后端时优雅降级
- 需要「localhost 全功能 + 公开页演示」的双态架构

## Boundaries · 不适用（Non-Goals）

- **不是通用 SaaS**：图床/热榜/AI 选题/知识库/违禁词扫描等**联网与密钥功能默认仅 localhost**（`server.py`），公开页走 Worker / 演示数据，**不碰真实密钥**。别指望公开页调用本机私有的 `/api/*`。
- **R2 图床只用 Cloudflare Worker 模式**：已**移除浏览器直传**（避免生成公开 `r2.dev` 链）。无 Worker/密钥时图床不可用，这是有意为之，不是 bug。
- **不打热榜稳定源**：知乎/微博/Reddit/X/Facebook 的公开热榜 API 在本机被限流/连不通，**别接**；只用心头/百度/B站/GitHub/HN 这些实测可用的。
- **红狐·抖音**与**小红书「七日爆款」**：前者需本机 key（¥0.06/次，仅 localhost）；后者接口认证过但**当前返回空**——**别接**，先用实测确认有数据再接入，否则空转扣费。
- **知识库/AI 选题仅 localhost**：公开页会提示「需本机 server.py」。
- **不交付后端**：本技能产出前端单页 + stdlib 服务器 + Worker，不引入 Node/Docker 后端（吸 insprira 时明确「不搬 Node/Docker」）。
- 相关能力请路由：仪表盘/前端实现见 `web-design-engineer`；静态站部署见 `cloudflare-pages-deploy`；海外受限网络下的 git/npm 见 `proxy-git-and-updates`。

## Workflow · 流程总览

1. **保底已知状态**：确认 `server.py`（stdlib）能在本机跑 `localhost:8080`；公开演示页已上线 `qingzuo.toolscomb.com`。
2. **改功能**：改 `index.html`（前端），新增工具务必同步两处——`showTab` 里的 `if(name==='xxx') renderXxx();` 和 keydown 的快捷键范围。
3. **改后端**：改 `server.py`（stdlib API），**launchd 托管时改完用 `launchctl kickstart -k` 重跑**。
4. **部署静态版**：`index.html` + `vendor/` → Cloudflare Pages；Worker → `npx wrangler deploy`。
5. **验证**：跑下方「验证 Trio」；公开页用 browser 测降级路径。
6. **收尾**：交付偏好见文末；本地/公开双态别混。

## 本地知识库（工具16，localhost-only）

`server.py` 加了一组 `/api/kb`（stdlib，零依赖）：`GET /api/kb`(列表)/`?id=`(读取)、`POST /api/kb`(保存/更新)、`DELETE /api/kb?id=`、`GET /api/kb/search?q=`(全文搜索)、`POST /api/kb/export {target}`(导出全部到目录)。数据存 `$CTB/data/kb/<id>.md`，**Obsidian frontmatter** 格式（title/type/tags/source/date/updated/id），可直接被 Obsidian 打开。前端「知识库」工具：左侧列表(类型筛选 chips：选题/成稿/拆书/素材) + 右侧编辑器(标题/类型/标签/来源/日期/正文/保存/删除)，顶部搜索框(300ms 防抖) + 「写新条」+「导出到 Obsidian」(目标目录在设置 → 知识库导出目录；作者默认 `~/BlinkLLMWiki-Release-v1.0/ContentKB`，**示例默认，可在设置里改**)。**集成**：热榜「存清单到知识库」(`hlKbSave`) 存为 选题；AI 选题每条「🕮」(`ai-kb`) 存为 选题；一键分发「存成稿到知识库」(`pbKbSave`) 存为 成稿。**仅 localhost**——公开页 kbLoad 会提示「需本机 server.py」。

## 新增（2026-08 参考 insprira 灵感熔炉）：趋势 / AI 选题 / 违禁词

参考仓库 `coracoo/insprira`（本地自媒体工作台），只吸它最值钱且能在轻量架构落地的三样，**不搬 Node/Docker**：

- **🔥 热点趋势（热榜 tool12 增强）**：server.py `/api/trend` 由每日快照 `$CTB/data/trend/<平台>/<yyyy-mm-dd>.json` 算每个词 **📈增长 / 🆕新上榜 / ➖稳定 / 📉冷却** 及在榜天数。快照在 `/api/hot` 抓取时写入（**首日快照优先，同日不覆盖**）；`snapshot_loop` 线程**启动即刻 + 每天 09:00 自动再快照**，连续几天攒出 7/14 天趋势。前端热榜顶部出现「🔥 趋势(N天)」筛选栏 + 每条趋势徽标。**趋势需历史数据——新部署首日全是「新上榜」属正常**，别误判为 bug。
- **🤖 AI 选题（热榜 tool12 下方面板）**：`POST /api/ai_topics`（body `{niche}`）→ 用**本地 DeepSeek**（`$CTB/config.secret.json` 的 `deepseek_api_key`，mode 600，**绝不进 index.html**；base 默认 `https://api.deepseek.com/v1`，model `deepseek-chat`）结合今日热榜证据生成 4–6 条选题 + 切入角度 + 匹配度。账号定位存 localStorage `ct-niche`（设置→AI 选题定位）。**仅 localhost**——公开页提示「需本机 server.py + 本地 key」。
...
**公开页演示模式（Demo，2026-08 上线）**：`index.html` 顶部 `const IS_DEMO = !['localhost','127.0.0.1'].includes(location.hostname)`。非 localhost 打开（**qingzuo.toolscomb.com** 为主演示版）自动进入演示模式：顶部「🧪 演示模式」横幅 + 侧栏运行状态=演示模式 + 默认落到热榜；热榜**直连 `cdn.toolscomb.com/hot` 秒开**（跳过死掉的 `/api/hot`——SPA 回退成 200 HTML）；AI 选题/知识库/AI 周报/抖音爆款扫描用内置**示例数据**（DEMO_TOPICS/DEMO_AWEEK/DEMO_REDFOX/DEMO_KB，均带「演示数据」标注，**不请求 /api/*、不碰任何真实密钥**，DeepSeek/红狐 key 仍只留本机）。localhost 打开 = 完整真实功能（走 /api/* + 本机 server.py）。改 demo 数据只动 index.html 顶部那组 `DEMO_*` 常量。

**launchd 托管（本机/作者专属）**：served by `~/Library/LaunchAgents/com.<USER>.content-toolbox.plist`（`<USER>`=你的 macOS 用户名）→ wrapper `~/.hermes/scripts/content-toolbox.sh`（sudo 无用，`launchctl bootstrap` 必须用户手动跑，agent 会被拦）。改 server.py 后用 `launchctl kickstart -k gui/$(id -u)/com.<USER>.content-toolbox` 重跑（kickstart agent 可跑；bootstrap 才被拦）。
...
本文档沉淀 2026-08 完整实战：从单文件构建 → R2 直传 → Cloudflare Worker → Cloudflare Pages 部署，含全部踩坑。**成品代码即本仓库**（`index.html` + `server.py` + `vendor/` + `worker/`），可直接参考。

## 分组导航 + 快捷键

侧栏分「工具」(1–8)、「图片」(9–11)、「选题·发布」(12–13)、「系统」(设置) 四组；快捷键 `1–9` 切前 9 个工具、`T` 切深浅色。新增工具后务必同步两处：`showTab` 里的 `if(name==='xxx') renderXxx();` 以及 keydown 的数组/范围 `e.key<='9'`。

## 依赖（vendored，别引 CDN）

- `qrcode.js`（基础库，全局 `qrcode` 函数）+ **`qrcode-utf8.js`（UTF-8 补丁，必须跟在其后加载）**——只有几百字节，`!function(qrcode){qrcode.stringToBytes=qrcode.stringToBytesFuncs['UTF-8']}(qrcode)`，不加载它中文会乱码。
...
- **热榜选题（工具12）**：聚合头条/B站/百度热榜，勾选进「选题清单」(localStorage `ct-hlpick`) 一键导出 Markdown。
...
- **红狐·抖音爆款源**：见 `references/redfox-douyin.md`（完整接入：后端函数、前端集成、配置、坑）。要点：`config.secret.json` 的 `redfox_api_key`（HEADER `REDFOX_API_KEY`，mode 600，**绝不进 index.html**）→ server.py `hot_redfox_douyin()` POST `https://redfox.hk/story/api/dy/search/likesRank`（每日热门作品榜 ¥0.06/次，实测 50→20 条）→ content/分类/@作者/点赞数。`api_trend` 的 `platform_order` 也要加 `redfox` 才能进趋势。
- **源的可达性（本机实测）**：知乎热榜 API 401、`zhihu-cli hot` 触发 `Code 30001 rate limit`、微博 `weibo.com/ajax/side/hotSearch` 403、**Reddit `r/all/hot.json` 本机连不通(code=000)**、**X 趋势需高级 API(tier 453)、Facebook 无公开趋势 API**——这些**都别当稳定源**。能用的：头条 `toutiao.com/hot-event/hot-board`、百度 `top.baidu.com/board`(HTML `<!--s-data:` 内嵌 JSON)、B站 `api.bilibili.com/x/web-interface/ranking/v2`(偶发限流，前端自动隐藏)、**GitHub `api.github.com/search/repositories?q=created:>7天&sort=stars`**（对效率/AI 号超对口）、**Hacker News firebase**（仅 Worker/公开页能用——本机 firebase 超慢 20s+/条，但 Cloudflare 快）。
...
- **工具 1 R2 图床（只保留 Worker 模式）**：worker.js + R2 绑定（免本机免 CORS，但依赖账号 Workers 能跑；很多账号 Workers 被官方卡死——见「关键坑 1」）。已**移除浏览器直传**（避免公开 `r2.dev` 链）。
...
`index.html` 结构：CSS 变量（浅/深色 `[data-theme]`）+ 左侧 nav（工具+设置）+ 右侧面板 Tab；**`.tab` 默认 `display:none`，`.tab.active` 才显示**（曾漏这条导致面板堆叠）。快捷键 `1-9` 切工具、`T` 切主题。
...
> **launchd 干净环境 SSL 坑（本机/作者专属）**：launchd 托管后 `/api/hot` 全空（`/api/kb`、`/api/trend` 正常），根因是本机网络对 HTTPS 做中间人拦截（自签名证书），launchd 干净环境 Python 默认 ssl 校验失败。修法：server.py 顶部 `import ssl; _SSL=ssl.create_default_context(); _SSL.check_hostname=False; _SSL.verify_mode=ssl.CERT_NONE`，`_get_json/_get_text` 和 DeepSeek 请求的 `urlopen(..., context=_SSL)`（公开数据端点，忽略证书校验安全）。改后 `launchctl kickstart -k gui/$(id -u)/com.<USER>.content-toolbox` 重跑。

### 3. R2 浏览器直传（SigV4，纯前端，历史方案）
...
> 注：当前版本已**移除浏览器直传模式**，此节作历史参考。若要恢复须重新配 R2 桶 CORS。
...
### 5. Cloudflare Worker 版（推荐，图床唯一托管）

`worker/worker.js`：路由 `POST /upload`(multipart: file+prefix)，`GET /view/<key>`(公开直链，Cache-Control immutable)，`GET /list`，`DELETE /delete?key=`；环境变量 `API_KEY` 保护写操作。`wrangler.toml`：
...
```bash
printf '%s' "$KEY" | npx wrangler secret put API_KEY   # 生成随机 key：openssl rand -hex 24
```
...
> **绑定 qingzuo.toolscomb.com（本机/作者专属）**：`POST /accounts/{acc}/pages/projects/content-toolbox/domains {"name":"qingzuo.toolscomb.com"}` → initializing→pending，再**手动建 CNAME**（用 `CF_DOMAIN_TOKEN`，它只覆盖 toolscomb.com zone）：`qingzuo.toolscomb.com → content-toolbox.pages.dev`（proxied）→ 等证书激活（pending→200，约几分钟）。
...
- **公开页**：browser_exec 开 `https://<proj>.pages.dev`，4 个静态工具各跑一次通过；再验证降级路径（趋势栏隐藏、AI 选题提示需 localhost）。

## 参考成品

- `index.html`（16 工具单文件，Tab + 纯 CSS/JS）
- `server.py`（localhost 服务器 + 每日快照线程）
- `worker/worker.js` + `wrangler.toml`（Worker 版 R2）
- `README.md`（用法 + 部署 + 当前状态）
- `references/redfox-douyin.md`（红狐·抖音爆款接入详解）
...
- **已验证**：本机 `server.py` localhost 全功能、公开演示页 qingzuo.toolscomb.com 上线、Worker `/hot` + `/view` 公开、R2 Worker 写保护（API_KEY）。
- **未验证/仅本机**：红狐·抖音爆款（需本机 key，仅 localhost）、launchd 托管（本机/作者专属）、知识库/AI 选题（需本机 server.py）。本文为 E1 结构文档 + 本机运行事实；跨环境复现需按「验证 Trio」实测，勿将此文档当作独立运行证明。
